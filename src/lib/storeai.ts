...
export async function getRecord(key: string) {
  return request<{ id: string; key: string; data: unknown } | null>(
    "GET",
...
export async function deleteRecord(id: string) {
  return request<{ success: boolean }>("DELETE", `/api/records/${id}`);
}
...
export async function listRecords(params?: Record<string, string>) {
  const query = params
    ...
    : "";
  return request<{
    records: Array<{ id: string; key: string; data: unknown }>;
