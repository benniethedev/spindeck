...
import type { SubmissionGenre, ContentRating } from '@/types';

const GENRES: SubmissionGenre[] = [
  'house', 'tech_house', 'techno', 'drum_and_bass',
  'ambient', 'lo_fi', 'hip_hop', 'rnb', 'pop',
  'afrobeats', 'afro_house', 'progressive_house',
  'trance', 'dubstep', 'industrial', 'other',
];
...
export default function SubmissionForm({ onSuccess, onCancel }: SubmissionFormProps) {
  ...
  const [rating, setRating] = useState<ContentRating>('clean');
  const [notes, setNotes] = useState('');
  const [links, setLinks] = useState<{ label: string; url: string }[]>([DEFAULT_LINK]);
  ...
  const addLink = () => setLinks([...links, { ...DEFAULT_LINK }]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!bpm) { setError('BPM is required'); return; }
