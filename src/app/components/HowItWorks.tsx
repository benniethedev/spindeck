          ...
          {steps.map((item, index) => (
            <div key={item.step} className="relative text-center">
              ...
              {index < steps.length - 1 && (
                <div className="hidden md:block absolute top-10 left-[60%] w-[80%] border-t-2 border-dashed border-zinc-200 dark:border-zinc-800" />
              )}
              ...
              <div className="relative inline-flex items-center justify-center w-20 h-20 rounded-2xl gradient-bg text-white mb-6 shadow-lg shadow-violet-500/20">
                {item.icon}
                <span className="absolute -top-2 -right-2 w-7 h-7 bg-amber-400 text-amber-900 text-xs font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-zinc-950">
                  {item.step}
                </span>
              </div>
